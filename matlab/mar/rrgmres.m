function [x, varargout] = rrgmres(A, b, varargin)
% An implementation for `RRGMRES`. RRGMRES solves the linear system
%       Ax = b 
% 
% or the least squares problem 
%       min || Ax - b ||_2 
% 
% RRGMRES finds the solution x which is belong to K(A,Ab). 
%
% 
% Syntaxes
% --------------
% x = rrgmres(A, b) 
% x = rrgmres(A, b, tol) 
% x = rrgmres(A, b, tol, maxit) 
% x = rrgmres(A, b, tol, maxit, restart)
% x = rrgmres(A, b, tol, maxit, restart, x0) 
% x = rrgmres(A, b, tol, maxit, restart, x0, version)
% x = rrgmres(A, b, tol, maxit, restart, x0, version, criteria)
% 
% 
% [x, exitflag] = rrgmres(__)
% [x, exitflag, resvec] = rrgmres(__) 
% [x, exitflag, resvec, Aresvec] = rrgmres(__) 
%
%
% Parameters
% ------------ 
...
%                   {'x0', 'version'}. 'x0' means initial guess; 'version' 
%                   determines the implementation of RRGMRES, 1 means simpler 
...
if version == 1
    inner_rrgmres = @rrgmresv1;
else
    inner_rrgmres = @rrgmresv2;
end
...
if ~isempty(restart)
    ...
    last_idx = 0; 
    for k = 1:maxit
        [x0, exitflag, resvec1, Aresvec1] = inner_rrgmres(A, b, tol, restart, x0, criteria);
    ...
    [x, exitflag, resvec, Aresvec] = inner_rrgmres(A, b, tol, maxit, x0, criteria);
end
...
function [x, exitflag, resvec, Aresvec] = rrgmresv1(A, b, tol, maxit, x0, criteria)
m = size(A, 1);
...
for j = 1:maxit
    ...
    % use modified Gram-schmidt process to update (I -V*V')*r0, otherwise,
    % it will be of loss accuracy
    xi = v' * rc;
    rc = rc - xi * v;
...
function [x, exitflag, resvec, Aresvec] = rrgmresv2(A, b, tol, maxit, x0, criteria)
m = size(A, 1);
...
v = A * v1; h11 = v1' * v;
v = v - h11 * v1; h12 = norm(v); 
v2 = v / h12;

gc = zeros(maxit + 2, 1); gs = gc;
gct = gc; gst = gc;

V = zeros(m, maxit + 2); W = V;
H = zeros(maxit + 2, maxit + 1);
z = gc;

[G, y] = planerot([h11; h12]);
